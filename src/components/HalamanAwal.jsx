        ...
        <div className="grid w-full items-center gap-8 lg:grid-cols-2 lg:gap-10">
          <div className="order-2 lg:order-1">
            ...
            <h1 className="mt-5 max-w-2xl text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl">
              Dashboard Monitoring Banjir yang lebih modern, jelas, dan
              responsif.
            </h1>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              <FiturLanding
                icon={<Activity className="mb-3 h-5 w-5 text-cyan-300" />}
                judul="Live status"
                deskripsi="Pantau kondisi terbaru secara cepat."
              />
              <FiturLanding
                icon={<Smartphone className="mb-3 h-5 w-5 text-cyan-300" />}
                judul="Mobile ready"
                deskripsi="Nyaman dibuka dari layar kecil."
              />
              <FiturLanding
                icon={<BarChart3 className="mb-3 h-5 w-5 text-cyan-300" />}
                judul="Grafik interaktif"
                deskripsi="Filter tanggal, bulan, dan tahun."
              />
            </div>
...
function FiturLanding({ icon, judul, deskripsi }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-md">
      {icon}
      <p className="text-sm font-semibold text-white">{judul}</p>
      <p className="mt-1 text-sm text-slate-400">{deskripsi}</p>
    </div>
  );
}
      ...
      <div className="overflow-hidden rounded-[32px] border border-white/10 bg-white/10 p-4 shadow-2xl backdrop-blur-xl sm:p-5">
        <div className="rounded-[28px] border border-white/10 bg-slate-950/70 p-4 sm:p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            />
            <InfoPreview label="Update" nilai="Realtime" />
            <InfoPreview label="Akses" nilai="HP & Laptop" />
          </div>
        </div>
      </div>
    </div>
...
function InfoPreview({ label, nilai, className = "text-white" }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <p className="text-xs uppercase tracking-[0.2em] text-slate-400">
        {label}
      </p>
      <p className={`mt-2 text-lg font-bold ${className}`}>{nilai}</p>
    </div>
  );
}